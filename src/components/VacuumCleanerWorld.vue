...
import RandomReflexAgent from '@/ai/vacuum-cleaner-world/RandomReflexAgent'
...
const agentOptions = new Map([
  ...
  ['random', { label: 'Random reflex agent', factoryMethod: RandomReflexAgent }]
])
...
export default {
  ...
  methods: {
    runStats () {
      const worldModelBasedAgent = createWorld(this.selectedDirtAppearanceProbability, this.selectedAgent)
      for (let i = 0; i < 100; i++) {
        worldModelBasedAgent.performNextAction()
      }
      console.log(worldModelBasedAgent.environment.dirtyCount)
  ...
  .root
    ...
    .grid-container
      ...
      .control
        ...
        ElButton(type='warning' size='large' round, @click='runStats') Run stats
